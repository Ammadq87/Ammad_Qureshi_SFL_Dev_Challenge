...
/**
 * Holds the defualt links for the navbar. If the navbar links
 * need to be updated, the client can send new links to be used.
 * 
 */
const defaultLinks = [
  {text: 'Home', url: ''},
...
const Navbar = ({links = defaultLinks}) => (
    ...
    style={{
      ...
      background: 'rgb(112,38,185)',
      color: 'white',
      height: '65px'
    }}
  >

    {/* Iterates through Links list to display links */}
    {
        links.map((link, i) => (
            <Link
              key={i}
    ...
    <img
      alt="Gatsby logo"
      height={20}
      style={{ margin: 0, float: 'right', color: 'white'}}
